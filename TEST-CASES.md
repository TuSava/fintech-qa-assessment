...
* **Preconditions:** Current server time $T$. Webhook payload timestamp set to $T - 301\text{s}$.
...
* **Expected Result:**
  * Math is exact: final balance exactly equals $Initial + (100 \times 1) - (100 \times 1) = Initial$.
...
* **Steps:**
  ...
  2. Calculate $\sum \text{Debit} - \sum \text{Credit}$.
* **Expected Result:**
  * $\sum \text{Debit} - \sum \text{Credit} \equiv 0$ with zero imbalance.
